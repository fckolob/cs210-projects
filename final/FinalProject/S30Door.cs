public class S30Door : Swinging
{

    public override string AdministrationWorkSheet()
    {
        throw new System.NotImplementedException();
    }

    public override string ProductionWorkSheet()
    {
        throw new System.NotImplementedException();
    }

    public override string ToSavedString()
    {
        throw new System.NotImplementedException();
    }

    public override string ToString()
    {
        throw new System.NotImplementedException();
    }

    public S30Door(System.String nameCustomer, System.String addressCustomer, int phoneCustomer, double totalPrice, double amountPaid, int widthMm, int heightMm, System.String color, int glassThickness, System.String openingDirection, System.String outOrIn, System.String design, bool doubleShash, bool dvh, bool perimeterFrame, bool buriedFrame): base( nameCustomer, addressCustomer,  phoneCustomer,  totalPrice,  amountPaid,  widthMm, heightMm, color, glassThickness, openingDirection, outOrIn,  design, doubleShash, dvh, perimeterFrame, buriedFrame)
    {
        throw new System.NotImplementedException();
    }

    public S30Door(System.String savedString): base(savedString)
    {
        throw new System.NotImplementedException();
    }
}